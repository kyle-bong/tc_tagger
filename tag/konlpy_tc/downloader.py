...
from konlpy_tc import internals
...
def default_download_dir():
    ...
    konlpy_tcdir = internals.get_datadir()
    ...
    return os.path.join(homedir, 'konlpy_tc_data')
...
class Downloader(object):
    ...
    PACKAGE_URL = 'http://konlpy_tc.github.io/konlpy_tc-data/packages/%s.%s'
    SCRIPT_URL = 'http://konlpy_tc.github.io/konlpy_tc-data/packages/%s.sh'
    INDEX_URL = 'http://konlpy_tc.github.io/konlpy_tc-data/index.json'
    ...
    def download(self, id=None, download_dir=None):
        ...
        if id is None:
            raise ValueError("Please specify a package to download. To download all available packages, pass 'all' to the argument: ``konlpy_tc.download('all')``.")
        elif id == 'all':
    ...
    def _download_package(self, info, download_dir, force=False):
        ...
        if os.path.exists(filepath):
            if status == self.STALE:
                yield "[konlpy_tc_data] This file is stale"
            os.remove(filepath)
        ...
        try:
            yield "[konlpy_tc_data] Downloading package '%s'..." % info['id']
            # TODO: progress bar
            urllib.urlretrieve(url, filepath)
        except IOError as e:
            yield "[konlpy_tc_data] Error downloading file"
            return
        yield "[konlpy_tc_data] Download finished"

        # If it's a zipfile, uncompress it.
        ext = os.path.splitext(filepath)[-1]
        if ext in ['.zip', '.tar']:
            yield "[konlpy_tc_data] Unzipping file %s" % filepath
