...
from konlpy_tc import utils
from konlpy_tc.constants import DATA_DIR, make_dir
...
#: A list of directories where the konlpy_tc data package might reside.
...
path += [d for d in os.environ.get('konlpy_tc_DATA', '').split(os.pathsep) if d]
if os.path.expanduser('~/') != '~/':
        path += [os.path.expanduser('~/konlpy_tc_data')]

# Windows common locations
if sys.platform.startswith('win'):
    path += [
        r'C:\konlpy_tc_data', r'D:\konlpy_tc_data', r'E:\konlpy_tc_data',
        os.path.join(sys.prefix, 'konlpy_tc_data'),
        os.path.join(sys.prefix, 'lib', 'konlpy_tc_data'),
        os.path.join(os.environ.get('APPDATA', 'C:\\'), 'konlpy_tc_data')]

# UNIX & OS X common locations
else:
    path += [
        '/usr/share/konlpy_tc_data',
        '/usr/local/share/konlpy_tc_data',
        '/usr/lib/konlpy_tc_data',
        '/usr/local/lib/konlpy_tc_data']

# Include konlpy_tc installpath
path += ['%s/data' % utils.installpath]


def find(resource_url):
    """Find the path of a given resource URL by searching through
    directories in ``konlpy_tc.data.path``.
    If the given resource is not found, raise a ``LookupError``,
    whose message gives a pointer to the installation instructions
    for ``konlpy_tc.download()``.
    ...
    :param resource_url: The URL of the resource to search for.
        ...
        converted to a platform-appropriate path separator by konlpy_tc.
    """
    for p in path:
    ...
    sep = '*' * 70
    msg = textwrap.fill(
        'Resource %s not found. Please use the konlpy_tc data downloader to obtain the resource: >>> konlpy_tc.download()' % resource_url,
        initial_indent='  ',
        subsequent_indent='  ',
        width=66)
    msg += '\n  Searched in:' + ''.join('\n  - %s' % p for p in path)
...
def load(resource_url, format='auto'):
    """Load a given resource from the konlpy_tc data package.
...
def clear():
    """clear the konlpy_tc output data directory
...
def listdir():
    """list konlpy_tc default data directory.
...
class CorpusReader(object):
    def __init__(self, extension='.txt'):
        """CorpusReader reads corpuses in konlpy_tc data directory.
            extension (str, optional): Defaults to '.txt'. extension of corpus to load.

        .. code-block:: python

            >>> from konlpy_tc.data import CorpusReader
            >>> reader = CorpusReader()
            >>> reader.read()
            >>> reader.corpus
            {...}
